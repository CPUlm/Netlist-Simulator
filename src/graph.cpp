...
template<class T>
void add_node(Graph<T> &graph, const T &label) {
    graph.nodes.emplace_back(label, NotVisited, {}, {});
}

template<class T>
Node<T> &node_of_label(Graph<T> &graph, const T &label) {
...
template<class T>
void add_edge(Graph<T> &graph, const T &id1, const T &id2) {
    try {
...
template<class T>
void clear_marks(Graph<T> &graph) {
    for (Node<T> &node: graph.nodes) {
...
template<class T>
std::vector<Node<T> &> find_roots(Graph<T> &graph) {
...
template<class T>
bool has_cycle(Graph<T> &graph);

template<class T>
std::list<Node<T>> topological(Graph<T> &graph);
