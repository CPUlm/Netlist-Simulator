...
std::vector<Equation> read_exp(Equation * e)	{
	Expression* exp = e->get_expression();
	if (exp->get_kind() == Expression::Kind::NOT)	{
		Value* v = static_cast<NotExpression*>(exp)->get_value();
...
void compile_file(std::string_view file_name, std::string_view file_content) {
  ...
  Program program = parser.parse_program();
	...
	for (Equation* equa : program.get_equations())	{
		read_exp(equa);
	}
