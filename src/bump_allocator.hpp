#ifndef NETLIST_BUMP_ALLOCATOR_HPP
#define NETLIST_BUMP_ALLOCATOR_HPP

#include <memory_resource>

/// This class implements a bump allocator.
///
/// From Rust <a href=https://docs.rs/bumpalo/latest/bumpalo/>bumpalo</a>
/// package documentation: <quote> Bump allocation is a fast, but limited
/// approach to allocation. We have a chunk of memory, and we maintain a pointer
/// within that memory. Whenever we allocate an object, we do a quick check that
/// we have enough capacity left in our chunk to allocate the object and then
/// update the pointer by the object’s size. That’s it!
///
...
/// allocations. That is, a group of objects that will all be allocated during
/// the same program phase, used, and then can all be deallocated together as a
/// group.
/// </quote>
class BumpAllocator {
public:
...
#endif // NETLIST_BUMP_ALLOCATOR_HPP
