...
static const char *char_repr[256] = {
...
std::string_view Utilities::value_to_str(value_t v, bus_size_t size) {
  ...
  s = char_repr[(v >> 8) & 0xff];
  std::copy(s, s + 8, buf + 48);

  s = char_repr[(v >> 16) & 0xff];
  std::copy(s, s + 8, buf + 40);

  s = char_repr[(v >> 24) & 0xff];
  std::copy(s, s + 8, buf + 32);

  s = char_repr[(v >> 32) & 0xff];
  std::copy(s, s + 8, buf + 24);

  s = char_repr[(v >> 40) & 0xff];
  std::copy(s, s + 8, buf + 16);

  s = char_repr[(v >> 48) & 0xff];
  std::copy(s, s + 8, buf + 8);

  s = char_repr[(v >> 56) & 0xff];
  std::copy(s, s + 8, buf);

  return {buf + 64 - size, size};
}

std::string Utilities::read_file(const ReportContext &ctx, std::string_view path) {
