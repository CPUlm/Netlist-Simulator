...
Simulator::MemoryMapper::MemoryMapper(const Program::ptr &p)
    : current_index(0) {
...
void Simulator::MemoryMapper::visit_reg_expr(const RegExpression &expr) {
  if (!reg_info.contains(
          expr.get_variable())) { // Can have multiple REG(o) in the program.
...
void Simulator::MemoryMapper::visit_ram_expr(const RamExpression &expr) {
  ...
  ram_info.emplace(
      std::piecewise_construct, std::forward_as_tuple(current_var->get_name()),
      std::forward_as_tuple(expr.get_write_enable(), expr.get_write_address(),
                            expr.get_write_data(), current_index, block_size,
                            expr.get_bus_size()));
...
Simulator::ExpressionEvaluator::ExpressionEvaluator(
    const Simulator::var_env &env, const std::vector<value_t> &mem,
    const Simulator::MemoryMapper &mem_map)
    : env(env), current_value(0), mem(mem), mem_map(mem_map) {}

constexpr value_t one = 1;

value_t Simulator::ExpressionEvaluator::eval(const Variable::ptr &var,
...
void Simulator::ExpressionEvaluator::visit_not_expr(const NotExpression &expr) {
  ...
  current_value =
      (~current_value) &
      Bus::max_value(expr.get_bus_size()); // Not operator applied here.
}

void Simulator::ExpressionEvaluator::visit_binop_expr(
    const BinOpExpression &expr) {
  ...
  case BinOpExpression::BinOpKind::NAND: {
    current_value =
        (~(lhs_value & current_value)) & Bus::max_value(expr.get_bus_size());
    return;
  }
  }
}
...
void Simulator::ExpressionEvaluator::visit_rom_expr(const RomExpression &expr) {
  ...
  size_t mem_addr =
      mem_map.rom_info.at(current_var->get_name()).block_index + current_value;
...
void Simulator::ExpressionEvaluator::visit_ram_expr(const RamExpression &expr) {
  visit(expr.get_read_address());
  size_t mem_addr =
      mem_map.ram_info.at(current_var->get_name()).block_index + current_value;
  current_value = mem[mem_addr];
}

void Simulator::ExpressionEvaluator::visit_concat_expr(
    const ConcatExpression &expr) {
...
void Simulator::ExpressionEvaluator::visit_select_expr(
    const SelectExpression &expr) {
  visit(expr.get_argument());
  current_value = (current_value >> expr.get_index()) & one;
}

Simulator::Simulator(const ReportContext &ctx, const InputManager &in_manager,
                     const Program::ptr &p)
    : prog(p), ctx(ctx), dep_list(Scheduler::schedule(ctx, p)), mem_map(p),
      memory(mem_map.current_index, 0), env(), expr_eval(env, memory, mem_map) {
  ...
  for (const auto &[chunk_name, memory_pair] : in_manager.memory_blocks) {
    ...
    if (mem_map.ram_info.contains(chunk_name)) {
      ...
      ctx.report(ReportSeverity::WARNING)
          .with_message("The memory chunk ", chunk_name,
                        " given as input is unused.")
          .with_code(50)
          .build()
          .print();
      continue;
    ...
    if (block_size != memory_chunks.size()) {
      ctx.report(ReportSeverity::ERROR)
          .with_message("Expected memory chunk size of ", block_size,
                        " for variable ", chunk_name, ". Given chunk size is ",
                        memory_chunks.size(), ".")
          .with_code(51)
          .build()
          .exit();
    ...
    if (value_size != input_value_size) {
      ctx.report(ReportSeverity::ERROR)
          .with_message("Expected value of size ", value_size,
                        " for memory chunk ", chunk_name,
                        ". Given value size is ", input_value_size, ".")
          .with_code(52)
          .build()
          .exit();
    }

    std::copy(memory_chunks.begin(), memory_chunks.end(),
              memory.begin() + block_index);
  ...
  for (const ident_t &chunk_name : uninitialised_blocks) {
    if (mem_map.rom_info.contains(chunk_name)) {
      ...
      ctx.report(ReportSeverity::ERROR)
          .with_message("The ROM memory chunk ", chunk_name,
                        " is not initialised.")
          .with_code(53)
          .build()
          .exit();
      ...
      ctx.report(ReportSeverity::WARNING)
          .with_message("The RAM memory chunk ", chunk_name,
                        " is not initialised.")
          .with_code(54)
          .build()
          .print();
